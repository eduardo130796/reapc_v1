...
export default function Contrato() {
  ...
  const fetchData = async (showRefresh = false) => {
    ...
    try {
      const [contratoData, cargosData] = await Promise.all([
        buscarContrato(id),
        listarCargos(id)
      ]);
           ...
           <div className="flex items-center gap-4">
              ...
              <div>
                 ...
                 <p className="text-slate-500 font-bold text-[10px] uppercase tracking-widest mt-0.5">
                   {cargos.length > 0 ? `${cargos.length} cargo${cargos.length > 1 ? "s" : ""} vinculado${cargos.length > 1 ? "s" : ""}` : "Nenhum cargo vinculado"}
                 </p>
              </div>
           </div>
